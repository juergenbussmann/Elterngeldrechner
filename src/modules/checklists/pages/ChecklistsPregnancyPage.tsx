...
import { schwangerschaftItems } from '../domain/checklistItems';
...
const CHECKLIST_SCHWANGERSCHAFT_KEY = 'checklistSchwangerschaftState.v1';
...
export const ChecklistsPregnancyPage: React.FC = () => {
  const { t } = useI18n();
  const { isPlus } = useBegleitungPlus();
  const { goTo } = useNavigation();
  const { openBottomSheet } = usePanels();
  const [state, setState] = React.useState<ChecklistState>(() => {
    const template = schwangerschaftItems.map((i) => ({ id: i.id, label: i.labelKey }));
    return getInitialState(template);
  });

  React.useEffect(() => {
    const template = schwangerschaftItems.map((i) => ({ id: i.id, label: i.labelKey }));
    const loaded = loadCustomState(CHECKLIST_SCHWANGERSCHAFT_KEY);
    const merged = mergeWithTemplate(loaded, template);
    setState(merged);
    saveCustomState(CHECKLIST_SCHWANGERSCHAFT_KEY, merged);
  }, []);

  useEffect(() => {
    if (!isPlus) return;
    const systemDef = buildSystemDef(
      SYS_IDS.schwangerschaft,
      t('nav.pregnancy'),
      [...schwangerschaftItems],
      t,
    );
  ...
  const handleBearbeiten = async () => {
    if (!isPlus) {
      openBegleitungPlusUpsell({ reason: 'checklists_edit_detail_pregnancy', feature: 'EXPORT' });
      return;
    }
    try {
      const systemDef = buildSystemDef(
  ...
  const handleToggle = (itemId: string) => {
    setState((prev) => {
      ...
      saveCustomState(CHECKLIST_SCHWANGERSCHAFT_KEY, next);
  ...
  return (
    <div className="screen-placeholder checklists-screen">
      <ChecklistRubricSection
        ...
        items={schwangerschaftItems}
        state={state}
        onToggle={handleToggle}
        onReset={handleReset}
        t={t}
        extraAfterItemId="erstausstattung"
        extraAfterContent={
          <div className="ui-text-link">
            <button
              type="button"
              onClick={() => goTo('/checklists/schwangerschaft/erstausstattung-neugeborenes')}
              className="ui-link-reset"
            >
              Erstausstattung öffnen
            </button>
          </div>
        }
      />
