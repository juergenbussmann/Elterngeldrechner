/**
 * Bildschirm-Vorschau der Antrags-Ausfüllhilfe — gleiche Texte und Reihenfolge wie im PDF (formularnaher Hauptteil, dann Hinweise, Anhang, fehlende Angaben).
 */
...
> = ({ model }) => {
  ...
  return (
    <>
      ...
      <Card className="still-daily-checklist__card">
        <h4 className="elterngeld-step__section-title">{SECTION_A_TITLE}</h4>
        {model.mainDocumentFlow.map((block, blockIdx) => {
          if (block.kind === 'form_subsection' || block.kind === 'official_form_only') {
            const sub = block.subsection;
            return (
              <React.Fragment key={`${block.kind}-${sub.subsectionTitle}-${blockIdx}`}>
          ...
          if (block.kind === 'month_distribution') {
            return (
              <React.Fragment key={`month-${blockIdx}`}>
                <p className="elterngeld-step__section-title elterngeld-step__section-title--nested">
                  {SUBSECTION_MONTHLY_SPLIT}
                </p>
                {hasMonthBezug ? (
          ...
          return (
            <React.Fragment key={`calc-${blockIdx}`}>
              <p className="elterngeld-step__section-title elterngeld-step__section-title--nested">
                {SUBSECTION_CALCULATION}
              </p>
              {model.calculation ? (
      ...
      <Card className="still-daily-checklist__card">
        ...
        <ul className="elterngeld-step__doc-list elterngeld-step__doc-list--checklist">
          {filterChecklistItemsForUnterlagenDisplay(model.checklistItems).map((item, i) => (
      ...
      <Card className="still-daily-checklist__card">
        <h4 className="elterngeld-step__section-title">{SECTION_MISSING_FOR_APPLICATION_TITLE}</h4>
        <p className="elterngeld-step__hint elterngeld-summary__forms-hint">{SECTION_MISSING_FOR_APPLICATION_INTRO_ANGABEN}</p>
        <ul className="elterngeld-step__doc-list elterngeld-step__doc-list--checklist">
          {antragsangabenNichtInApp.map((item) => (
            <li key={item}>{item}</li>
          ))}
        </ul>
        <p className="elterngeld-step__hint elterngeld-summary__forms-hint">
        ...
        <ul className="elterngeld-step__doc-list elterngeld-step__doc-list--checklist">
          {SECTION_MISSING_FOR_APPLICATION_FURTHER_ITEMS.map((item) => (
            <li key={item}>{item}</li>
          ))}
        </ul>
        <p className="elterngeld-step__section-title elterngeld-step__section-title--nested">
        ...
        <ul className="elterngeld-step__doc-list elterngeld-step__doc-list--checklist">
          {SECTION_MISSING_PLANNING_COVERED_ITEMS.map((item) => (
            <li key={item}>✔ {item}</li>
          ))}
        </ul>
      </Card>
