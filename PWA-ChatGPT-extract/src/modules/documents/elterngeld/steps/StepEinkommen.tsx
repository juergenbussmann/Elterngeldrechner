/**
 * Schritt Einkommen – zeigt nur die Einkommensfelder.
 * Verwendet ausschließlich parentA.incomeBeforeBirth und parentB.incomeBeforeBirth.
 */

import React from 'react';
import { Card } from '../../../../shared/ui/Card';
import { TextInput } from '../../../../shared/ui/TextInput';
import type { ElterngeldApplication } from '../types/elterngeldTypes';
import { EMPTY_ELTERNGELD_PARENT } from '../types/elterngeldTypes';

type Props = {
  values: ElterngeldApplication;
  onChange: (values: ElterngeldApplication) => void;
};

export const StepEinkommen: React.FC<Props> = ({ values, onChange }) => {
  const updateParentA = (v: string) => {
    onChange({
      ...values,
      parentA: { ...values.parentA, incomeBeforeBirth: v },
    });
  };

  const updateParentB = (v: string) => {
    const parentB = values.parentB ?? EMPTY_ELTERNGELD_PARENT;
    onChange({
      ...values,
      parentB: { ...parentB, incomeBeforeBirth: v },
    });
  };

  const showPartner = values.applicantMode === 'both_parents' && values.parentB;

  return (
    <>
      <Card className="still-daily-checklist__card">
        <h3 className="elterngeld-step__title">Mutter</h3>
        <label className="elterngeld-step__label">
          <span>Nettoeinkommen vor Geburt</span>
          <TextInput
            type="text"
            value={values.parentA.incomeBeforeBirth}
            onChange={(e) => updateParentA(e.target.value)}
            placeholder="z. B. 2500"
          />
        </label>
      </Card>

      {showPartner && (
        <Card className="still-daily-checklist__card">
          <h3 className="elterngeld-step__title">Partner / Partnerin</h3>
          <label className="elterngeld-step__label">
            <span>Nettoeinkommen vor Geburt</span>
            <TextInput
              type="text"
              value={values.parentB!.incomeBeforeBirth}
              onChange={(e) => updateParentB(e.target.value)}
              placeholder="z. B. 2500"
            />
          </label>
        </Card>
      )}

      <p className="elterngeld-step__hint elterngeld-step__hint--section">
        Das Elterngeld beträgt ungefähr 65 % des Nettoeinkommens.
      </p>
    </>
  );
};
